...
export async function registerRoutes(
  ...
  // ── NFT Metadata (ARC-3) ────────────────────────────────────────────────────
  // Registered FIRST so it is never shadowed by express.static or the Vite /
  // SPA catch-all routes added later in server/index.ts.
  // Public endpoint consumed by Algorand NFT marketplaces and wallets.
  // Returns immutable plot attributes only — no mutable game state.
  // BASE_URL: set PUBLIC_BASE_URL env var in production; falls back to the
  // request's own origin so the route works locally and on Replit without config.
